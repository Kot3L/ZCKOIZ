      ...
      @if (editing()) {
        <div class="comic-card">
          <h2 class="font-heading text-xl text-orange-primary mb-4">
            {{ form.id ? 'Edytuj artykuł' : 'Nowy artykuł' }}
          </h2>
          <form (submit)="saveItem($event)" class="space-y-4">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500 mt-1">Wybrany album zostanie wyświetlony w artykule jako karuzela zdjęć.</p>
            </div>
        ...
        <table class="w-full text-left text-sm">
          ...
          <tbody>
            @for (item of items(); track item.id) {
              ...
              <tr><td colspan="4" class="px-4 py-8 text-center text-gray-500">Brak artykułów</td></tr>
            }
          </tbody>
        </table>
      </div>
...
export class EdukacjaMundurowaAdminComponent implements OnInit {
  ...
  async saveItem(event: Event) {
    ...
    try {
      ...
      // PDF wgrany z urządzenia trzymamy w osobnej kolekcji,
      // żeby nie przekraczać limitu 1 MiB dokumentu `education_articles`.
