...
import { EducationArticle, EducationPage, GalleryImage } from '../../core/models/database.types';
      ...
      <div class="container-main max-w-4xl">
        @if (loading()) {
          ...
          @if (legacy()!.cover_image_url) {
            <figure class="mb-10 max-h-[32rem] overflow-hidden rounded-xl bg-cream-dark flex justify-center">
              <img [src]="legacy()!.cover_image_url" [alt]="legacy()!.title" class="w-full h-auto max-h-[32rem] object-contain" />
            </figure>
          }
          ...
          @if (legacy()!.content_images?.length) {
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
              @for (imgUrl of legacy()!.content_images; track imgUrl) {
                <figure class="overflow-hidden rounded-xl bg-cream-dark flex justify-center">
                  <img [src]="imgUrl" [alt]="legacy()!.title" class="w-full max-h-[26rem] object-contain" loading="lazy" />
                </figure>
              }
            </div>
          }
...
export class EdukacjaMundurowaComponent implements OnInit {
  item = signal<EducationArticle | null>(null);
  ...
  async ngOnInit() {
    ...
    try {
      let data = await this.fb.getEducationArticleBySlug(slug);
      if (!data && slug === 'edukacja-mundurowa') {
      ...
      if (data?.pdf_name && !data.pdf_url) {
        const pdf = await this.fb.getEducationPdf(data.id);
        if (pdf?.data) {
